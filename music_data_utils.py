...
import urllib.parse, urllib.request, urllib.error, urllib.parse, os, midi, math, random, re, string, sys
import numpy as np
...
sources['classical']['beethoven'] = ['http://www.midiworld.com/beethoven.htm',
                                     'http://www.classicalmidi.co.uk/beethoven.htm']
...
sources['classical']['eshpai'] = ['http://www.classicalmidi.co.uk/Eshpai.htm',
...
sources['classical']['rachmaninov'] = ['http://www.midiworld.com/rachmaninov.htm',
                                       'http://www.classicalmidi.co.uk/rach.htm']
...
sources['classical']['scarlatti'] = ['http://www.midiworld.com/scarlatti.htm',
                                     'http://www.classicalmidi.co.uk/scarlatt.htm']
...
sources['classical']['schumann'] = ['http://www.midiworld.com/schumann.htm',
                                    'http://www.classicalmidi.co.uk/schuman.htm']
...
sources['classical']['tchaikovsky'] = ['http://www.midiworld.com/tchaikovsky.htm',
            ...
            for composer in sources[genre]:
                ...
                for url in sources[genre][composer]:
                    try:
                        ...
                        a = 'href="http://' + urlparsed.hostname + '/'
                        b = 'href="http://' + urlparsed.hostname + urlparsed.path[:urlparsed.path.rfind('/')] + '/'

                        data = re.sub("href=\"/", a, data, flags=re.IGNORECASE)
                        data = re.sub("href=\"(?!http:)", b, data,
                                      flags=re.IGNORECASE)
                        # if 'classicalmidi' in url:
                        #  print data
                        ...
                        for link in links:
                            ...
                            if os.path.exists(localpath):
                                ...
                                try:
                                    # response_midi = urllib.request.urlopen(link)
                                    try:
                                        os.makedirs(localdir)
                                    except:
                                        pass
                                    download(link, localpath)
                                    # data_midi = response_midi.read().decode()
                                    # if 'DOCTYPE html PUBLIC' in data_midi:
                                    #     print(
                                    #         'Seems to have been served an html page instead of a midi file. Continuing with next file.')
                                    # elif 'RIFF' in data_midi[0:9]:
                                    #     print(
                                    #         'Seems to have been served an RIFF file instead of a midi file. Continuing with next file.')
                                    # else:
                                    #     pass
                                except:
    ...
    """
        ...
        if self.single_composer is not None:
            ...
            for genre in self.genres:
                self.composers.extend(list(sources[genre].keys()))
            if debug == 'overfit':
        ...
        if select_validation_percentage or select_test_percentage:
            ...
            if select_test_percentage:
                ...
                print(('Selected validation set (FLAG --select_validation_percentage): {}'.format(
                    ...
                    if n < len(batch[s][SONG_DATA]):
                        ...
                        for simultaneous in range(1, self.tones_per_cell):
                            if n + simultaneous >= len(batch[s][SONG_DATA]):
                                break
                            if batch[s][SONG_DATA][n + simultaneous][BEGIN_TICK] - batch[s][SONG_DATA][n][
                                BEGIN_TICK] == 0:
...
def main():
    ...
    dl = MusicDataLoader(datadir="mididata", select_validation_percentage=0.0, select_test_percentage=0.0)
