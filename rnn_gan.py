...
model_layout_flags = ['num_layers_g', 'num_layers_d', 'meta_layer_size', 'hidden_size_g', 'hidden_size_d',
                      'biscale_slow_layer_ticks', 'multiscale', 'multiscale', 'disable_feed_previous', 'pace_events',
                      'minibatch_d', 'unidirectional_d', 'feature_matching', 'composer']
    ...
    if FLAGS.traindir:
        ...
        for arg in FLAGS.__flags:
            ...
            if FLAGS.ignore_saved_args and os.path.exists(os.path.join(saved_args_dir, arg + '.pkl')):
                print(
                    '{:%Y-%m-%d %H:%M:%S}: saved_args: Found {} setting from saved state, but using CLI args ({}) and saving (--ignore_saved_args).'.format(
                        datetime.datetime.today(), arg, getattr(FLAGS, arg)))
                ...
                with open(os.path.join(saved_args_dir, arg + '.pkl'), 'rb') as f:
                    ...
                    print('{:%Y-%m-%d %H:%M:%S}: saved_args: {} from saved state ({}), ignoring CLI args.'.format(
                        datetime.datetime.today(), arg, getattr(FLAGS, arg)))
            elif save_if_none_found:
                print(
                    '{:%Y-%m-%d %H:%M:%S}: saved_args: Found no {} setting from saved state, using CLI args ({}) and saving.'.format(
                        datetime.datetime.today(), arg, getattr(FLAGS, arg)))
                with open(os.path.join(saved_args_dir, arg + '.pkl'), 'wb') as f:
                    pkl.dump(getattr(FLAGS, arg), f)
            else:
                print(
                    '{:%Y-%m-%d %H:%M:%S}: saved_args: Found no {} setting from saved state, using CLI args ({}) but not saving.'.format(
                        datetime.datetime.today(), arg, getattr(FLAGS, arg)))
...
def minibatch(inp, num_kernels=25, kernel_dim=10, scope=None, msg='', reuse_scope=False):
    ...
    with tf.variable_scope(scope or 'minibatch_d') as scope:
        ...
        minibatch_features = tf.Print(minibatch_features,
                                      [tf.reduce_min(minibatch_features), tf.reduce_max(minibatch_features)],
...
class RNNGAN(object):
    ...
    def __init__(self, is_training, num_song_features=None, num_meta_features=None):
        ...
        with tf.variable_scope('G') as scope:
            scope.set_regularizer(tf.contrib.layers.l2_regularizer(scale=FLAGS.reg_scale))
            lstm_cell = tf.contrib.rnn.BasicLSTMCell(FLAGS.hidden_size_g, forget_bias=1.0, state_is_tuple=True)
            if is_training and FLAGS.keep_prob < 1:
                lstm_cell = tf.contrib.rnn.DropoutWrapper(
                    lstm_cell, output_keep_prob=FLAGS.keep_prob)
            cell = tf.contrib.rnn.MultiRNNCell([lstm_cell] * FLAGS.num_layers_g, state_is_tuple=True)
        ...
        pretraining_grads, _ = tf.clip_by_global_norm(tf.gradients(self.rnn_pretraining_loss, self.g_params),
                                                      FLAGS.max_grad_norm)
        self.opt_pretraining = g_optimizer.apply_gradients(list(zip(pretraining_grads, self.g_params)))
        ...
        d_grads, _ = tf.clip_by_global_norm(tf.gradients(self.d_loss, self.d_params),
                                            FLAGS.max_grad_norm)
        self.opt_d = d_optimizer.apply_gradients(list(zip(d_grads, self.d_params)))
        if FLAGS.feature_matching:
            ...
            g_grads, _ = tf.clip_by_global_norm(tf.gradients(self.g_loss, self.g_params),
                                                FLAGS.max_grad_norm)
        self.opt_g = g_optimizer.apply_gradients(list(zip(g_grads, self.g_params)))
    ...
    def discriminator(self, inputs, is_training, msg=''):
        ...
        lstm_cell = tf.contrib.rnn.BasicLSTMCell(FLAGS.hidden_size_d, forget_bias=1.0, state_is_tuple=True)
        if is_training and FLAGS.keep_prob < 1:
            lstm_cell = tf.contrib.rnn.DropoutWrapper(
                lstm_cell, output_keep_prob=FLAGS.keep_prob)
        cell_fw = tf.contrib.rnn.MultiRNNCell([lstm_cell] * FLAGS.num_layers_d, state_is_tuple=True)
        ...
        if not FLAGS.unidirectional_d:
            ...
            if is_training and FLAGS.keep_prob < 1:
                lstm_cell = tf.contrib.rnn.DropoutWrapper(
                    lstm_cell, output_keep_prob=FLAGS.keep_prob)
            cell_bw = tf.contrib.rnn.MultiRNNCell([lstm_cell] * FLAGS.num_layers_d, state_is_tuple=True)
            ...
            outputs, state = tf.contrib.rnn(cell_fw, inputs, initial_state=self._initial_state_fw)
            # endoutput = outputs[-1]
...
def run_epoch(session, model, loader, datasetlabel, eval_op_g, eval_op_d, pretraining=False, verbose=False,
              run_metadata=None, pretraining_d=False):
            ...
            if pretraining:
                print(
                    "{}: {} (pretraining) batch loss: G: {:.3f}, avg loss: G: {:.3f}, speed: {:.1f} songs/s, avg in graph: {:.1f}, avg in python: {:.1f}.".format(
                        datasetlabel, iters, g_loss, float(g_losses) / float(iters), songs_per_sec, avg_time_in_graph,
                        avg_time_in_python))
            else:
                print(
                    "{}: {} batch loss: G: {:.3f}, D: {:.3f}, avg loss: G: {:.3f}, D: {:.3f} speed: {:.1f} songs/s, avg in graph: {:.1f}, avg in python: {:.1f}.".format(
                        ...
                        float(d_losses) / float(iters), songs_per_sec, avg_time_in_graph, avg_time_in_python))
        # batchtime = time.time()
        [batch_meta, batch_song] = loader.get_batch(model.batch_size, model.songlength, part=datasetlabel)
...
def main(_):
    ...
    loader = music_data_utils.MusicDataLoader(FLAGS.datadir, FLAGS.select_validation_percentage,
                                              FLAGS.select_test_percentage, FLAGS.works_per_composer, FLAGS.pace_events,
                                              synthetic=synthetic, tones_per_cell=FLAGS.tones_per_cell,
                                              single_composer=FLAGS.composer)
    if FLAGS.synthetic_chords:
        # This is just a print out, to check the generated data.
        batch = loader.get_batch(batchsize=1, songlength=400)
        loader.get_midi_pattern([batch[1][0][i] for i in range(batch[1].shape[1])])
    ...
    with tf.Graph().as_default(), tf.Session(
            config=tf.ConfigProto(log_device_placement=FLAGS.log_device_placement)) as session:
        ...
        if FLAGS.initialize_d:
            ...
            if ckpt and tf.gfile.Exists(ckpt.model_checkpoint_path):
                ...
                session.run(
                    tf.initialize_variables([v for v in tf.trainable_variables() if v.name.startswith('model/D/')]))
            else:
            ...
            for i in range(global_step, FLAGS.max_epoch):
                ...
                if i < FLAGS.pretraining_epochs:
                    ...
                    train_g_loss, train_d_loss = run_epoch(session, m, loader, 'train', m.opt_pretraining, opt_d,
                                                           pretraining=True, verbose=True, run_metadata=run_metadata,
                                                           pretraining_d=FLAGS.pretraining_d)
                    if FLAGS.pretraining_d:
                        try:
                            print("Epoch: {} Pretraining loss: G: {:.3f}, D: {:.3f}".format(i, train_g_loss,
                                                                                            train_d_loss))
                        except:
                            print(train_g_loss)
                            print(train_d_loss)
                ...
                if FLAGS.exit_after > 0 and time.time() - train_start_time > FLAGS.exit_after * 60:
                    print("%s: Has been running for %d seconds. Will exit (exiting after %d minutes)." % (
                    datetime.datetime.today().strftime('%Y-%m-%d %H:%M:%S'), (int)(time.time() - train_start_time),
                    FLAGS.exit_after))
                    save = True
                    do_exit = True
                ...
                if save:
                    ...
                    if FLAGS.profiling:
                        ...
                        with open(os.path.join(plots_dir, 'timeline.json'), 'wb') as f:
                            f.write(ctf)
                ...
                with open(os.path.join(plots_dir, 'gnuplot-input.txt'), 'a') as f:
                    try:
                        ...
                        f.write('{} {} {} {} {} {}\n'.format(i, m.lr.eval(), train_g_loss, train_d_loss, valid_g_loss,
                                                             valid_d_loss))
                ...
                try:
                    ...
                    print('failed to run gnuplot. Please do so yourself: gnuplot gnuplot-commands.txt cwd={}'.format(
                        plots_dir))
                ...
                filename = os.path.join(generated_data_dir, 'out-{}-{}-{}.mid'.format(experiment_label, i,
                ...
                if len(stats):
                    ...
                    stats_keys = ['scale_score', 'tone_min', 'tone_max', 'tone_span', 'freq_min', 'freq_max',
                                  'freq_span', 'tones_unique', 'repetitions_2', 'repetitions_3', 'repetitions_4',
                                  'repetitions_5', 'repetitions_6', 'repetitions_7', 'repetitions_8', 'repetitions_9',
                                  'estimated_beat', 'estimated_beat_avg_ticks_off', 'intensity_min', 'intensity_max',
                                  'intensity_span', 'polyphony_score', 'top_2_interval_difference',
                    ...
                    if not os.path.exists(statsfilename):
                        with open(statsfilename, 'a') as f:
                            ...
                            f.write('# global-step {} {}\n'.format(
                                ' '.join([s.replace(' ', '_') for s in stats_keys_string]), ' '.join(stats_keys)))
                    with open(statsfilename, 'a') as f:
                        f.write('{} {} {}\n'.format(i, ' '.join(
                ...
                if do_exit:
                    if FLAGS.call_after is not None:
                        ...
                        datetime.datetime.today().strftime('%Y-%m-%d %H:%M:%S'), FLAGS.call_after))
                        res = call(FLAGS.call_after.split(" "))
                        print('{}: call returned {}.'.format(datetime.datetime.today().strftime('%Y-%m-%d %H:%M:%S'),
                                                             res))
                    exit()
                sys.stdout.flush()
